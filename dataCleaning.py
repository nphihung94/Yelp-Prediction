...
def handleYelpingSince(df, ys_col, new_attr, unit="year"):
    divider = 365
    if unit != "year":
...
if __name__ == '__main__':
    print("### Loading csv")
    bdf = pd.read_csv("business.csv")
    udf = pd.read_csv("users.csv")

    print("### Cleaning business data")
    bstart = time.time()
    
#    printNanRatio(bdf, "business data")
    high_nan_attrs = ["attributes_AcceptsInsurance", "attributes_AgesAllowed",
                      "attributes_DietaryRestrictions", 
                      "attributes_HairSpecializesIn", "attributes_Open24Hours", 
                      "attributes_RestaurantsCounterService"]
    
    unused_attrs = ["address", "attributes", "hours", "name", "neighborhood", 
                    "city", "state", "hours_Monday", "hours_Tuesday", 
                    "hours_Wednesday", "hours_Thursday", "hours_Friday", 
                    "hours_Saturday", "hours_Sunday", "postal_code", "latitude", 
                    "longitude"]
    
    clean_bdf = bdf.drop(high_nan_attrs + unused_attrs, axis=1)
    clean_bdf["attributes_RestaurantsPriceRange2"] = imputeDfCol(clean_bdf, 
                                     "attributes_RestaurantsPriceRange2", "mean")
    
    # Specially handled attrs
    clean_bdf = handleReviewCountBusiness(clean_bdf, "review_count", 
                                                      "heavily_reviewed", 500)
    ...
    for attr in attrWithOnlyBool:
        quantifyDf(clean_bdf, attr, {"True": 1.0, "False": 0.0}, "mean")
    quantifyDf(clean_bdf, "attributes_Alcohol", 
               {"full_bar": 1.0, "beer_and_wine": 1.0, "none": 0.0}, "mean")
    quantifyDf(clean_bdf, "attributes_BYOBCorkage", 
               {"yes_free": 1.0, "yes_corkage": 1.0, "no": 0.0}, "mean")
    quantifyDf(clean_bdf, "attributes_Smoking", 
               {"yes": 1.0, "outdoor": 1.0, "no": 0.0}, "mean")
    quantifyDf(clean_bdf, "attributes_WiFi", 
               {"free": 1.0, "paid": 0.75, "no": 0.0}, "mean")
    
    bend = time.time()
    print(f"# Took: {bend - bstart} sec")
    
    print("### Cleaning user data")
#    printNanRatio(udf, "users data")
    ustart = time.time()
    unused_attrs = ["name", "friends"]
    noisy_attrs = ["compliment_profile", "compliment_plain", "elite", "fans", "useful"]
    clean_udf = udf.drop(unused_attrs + noisy_attrs, axis=1)
    clean_udf = handleYelpingSince(clean_udf, "yelping_since", "yelping_for")
#    handleUserElite(clean_udf, "elite")
#    normalizeWith(clean_udf, "fans", 10)
    ...
    normalize_target_cols = ["compliment_cool", "compliment_cute", 
                             ...
                             #"compliment_plain", "useful"
                             "compliment_writer", "cool", "funny"]
    clean_udf = normalizeWithReviewCount(clean_udf, normalize_target_cols, "review_count")
    uend = time.time()
    print(f"# Took: {uend - ustart} sec")

    print("### Writing cleaned df to csv")
    clean_bdf.to_csv("business_cl.csv", index=False)
    clean_udf.to_csv("users_cl.csv", index=False)
    print("### All done")
